...
@ApplicationScoped
public class GameQPlatformParser extends ParsingFlow {

    private final Logger log = LoggerFactory.getLogger(GameQPlatformParser.class);

    @Override
    public GameQParsingStates executeWorkflow(GameQParsingStates currentState) {
        log.info("Starting GameQPlatformParser flow");

        return GameQParsingStates.PLATFORMS_PARSED;
    }
